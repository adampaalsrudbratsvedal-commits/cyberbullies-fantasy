...
def set_apb03_sid(db: Session = Depends(get_db)):
    ...
    all_info = [{"u": u.username, "fifa": u.fifa_username, "admin": u.is_admin} for u in users]
    ...
    if not user:
        return {"error": "ingen brukere i db", "all": all_info}
    user.fifa_username = "Apb03"
    user.fifa_sid = cookie
    db.commit()
    return {"ok": True, "username": user.username, "all": all_info}
